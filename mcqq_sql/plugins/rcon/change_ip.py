...
change_ip = on_command("修改rconip", rule=to_me(), permission=SUPERUSER, priority=3, block=True)
change_ip.__doc__ = """修改rconip Server IP（私聊丨超级用户）"""

change_ip.handle()(handle_server_name)

change_ip.got("server_name", prompt="请输入内容：服务器名 IP")(server_name_check)
...
async def _(event: PrivateMessageEvent, server_name: str = ArgPlainText("server_name")):
    ...
    if event.sub_type != "group":  # 不处理群临时会话
        ...
        if await db.update_server(
            ...
            await change_ip.finish(f"修改 [{server.server_name}] 的Rcon_IP成功")
