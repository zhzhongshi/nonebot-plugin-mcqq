...
from nonebot.adapters.onebot.v11 import Message, MessageEvent, GroupMessageEvent, Bot, GROUP_ADMIN, GROUP_OWNER
from nonebot.permission import SUPERUSER
from nonebot_plugin_guild_patch import ChannelDestroyedNoticeEvent, GuildMessageEvent

from . import config
from .config import get_mc_qq_mcrcon_guild_admin_roles
...
async def send_msg_to_qq(bot: Bot, json_msg):
    ...
    if config.get_mc_qq_display_server_name():
            ...
            if per_server['server_name'] == json_msg['server_name']:
                ...
                if per_server['guild_list']:
                    for per_guild in per_server['guild_list']:
                        guild = await db.get_guild(id=per_guild)
                        logger.success(
...
async def msg_process(bot: Bot, event: Union[GroupMessageEvent, GuildMessageEvent]):
    ...
    # 发送群聊名称
    message_type = {}
    if config.get_mc_qq_send_group_name():
        if isinstance(event, GroupMessageEvent):
